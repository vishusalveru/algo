"""
=============================================================
  Nifty 50 Scalping Bot v8 — Multi-Trade + Crash-Proof Build
  ─────────────────────────────────────────────────────────
  INHERITS : All v5 (F1-F31) + v6 (V6-F1 to V6-F15) +
             v7 (V7-F1 to V7-F17) fixes.
...
PER_STRATEGY_RULES = {
    ...
    "BOS": {
        "rsi_long_max"          : 85,   # very high — momentum confirms BOS
    ...
    # ORB+EMA — breakout strategy, moderate RSI gate
    "ORB+EMA": {
        "rsi_long_max"          : 72,
        "rsi_short_min"         : 28,
        "allow_counter_trend"   : False,
...
def get_log_filenames():
    ...
    date_str = datetime.date.today().strftime("%Y-%m-%d")
    return {
        "scan"  : f"scan_log_v8_{date_str}.csv",
        "trade" : f"trade_log_v8_{date_str}.csv",
        "skip"  : f"skip_log_v8_{date_str}.csv",
    }
...
def run():
    ...
    send_telegram(
        ...
        f"📋 <b>Commands:</b> /bias /status /report /help\n"
...
if __name__ == "__main__":
    try:
        ...
        log.info("Nifty Bot v8 stopped by user")
        send_telegram("⏹ Nifty Bot v8 stopped (KeyboardInterrupt)")
    except Exception as e:
        log.error(f"Fatal error: {e}", exc_info=True)
        send_telegram(f"🚨 Nifty Bot v8 CRASHED: {e}")

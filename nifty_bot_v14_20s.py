...
SCAN_COLS = ["datetime","nifty_ltp","atm","regime","rsi","atr","vix",
             ...
             # ── chop-analysis fields (2026-06-17) — measure before wiring ──
             "rvol","atr_slope"]
TRADE_COLS = ["trade_no","ts","strategy","direction","entry_nifty","entry_premium",
              ...
              "mfe_at_2min","mfe_at_5min","mfe_at_10min",
              # v2 state AT ENTRY (shadow instrumentation, audit fix B 2026-06-11)
              "entry_v2_strength","entry_v2_momentum","entry_v2_r2",
              # post-hoc analysis fields (2026-06-16) — for later correlation work
              "entry_gap_pct","stop_efficiency","mfe_to_mae_ratio"]
...
def init_logs():
    d = datetime.date.today().strftime("%Y-%m-%d")
    for fn, cols in [(f"scan_{TAG}_{d}.csv", SCAN_COLS), (f"trade_{TAG}_{d}.csv", TRADE_COLS),
                     (f"open_{TAG}_{d}.csv", OPEN_COLS), (f"skip_{TAG}_{d}.csv", SKIP_COLS)]:
        if not os.path.exists(fn):
            csv.DictWriter(open(fn,"w",newline=""), fieldnames=cols).writeheader()

def log_scan(rec):
    d = datetime.date.today().strftime("%Y-%m-%d")
    w = csv.DictWriter(open(f"scan_{TAG}_{d}.csv","a",newline=""), fieldnames=SCAN_COLS)
    w.writerow({c: rec.get(c,"") for c in SCAN_COLS})

def log_trade(rec):
    d = datetime.date.today().strftime("%Y-%m-%d")
    w = csv.DictWriter(open(f"trade_{TAG}_{d}.csv","a",newline=""), fieldnames=TRADE_COLS)
...
def log_open(pos, decision, vix):
    ...
    w = csv.DictWriter(open(f"open_{TAG}_{d}.csv","a",newline=""), fieldnames=OPEN_COLS)
    w.writerow({c: rec.get(c,"") for c in OPEN_COLS})

def log_skip(rec):
    """Blocked signals, broken out separately for easy review (per request)."""
    d = datetime.date.today().strftime("%Y-%m-%d")
    w = csv.DictWriter(open(f"skip_{TAG}_{d}.csv","a",newline=""), fieldnames=SKIP_COLS)
...
def run():
    ...
    while True:
        try:
            ...
            if pos is not None:
                ...
                if reason:
                    ...
                    log_trade({
                        ...
                        "mfe_at_2min":pos.mfe_at_2min if pos.mfe_at_2min is not None else "",
                        "mfe_at_5min":pos.mfe_at_5min if pos.mfe_at_5min is not None else "",
                        "mfe_at_10min":pos.mfe_at_10min if pos.mfe_at_10min is not None else "",
                        ...
                        # post-hoc analysis (2026-06-16):
                        # gap% at entry (knowable at open — valid forward context)
                        "entry_gap_pct":round(gap_pct,2),
                        # stop_efficiency: for losers, actual loss / max-possible
                        # loss (SL distance). ~1.0 = stop worked; >1 = overshoot/
                        # execution problem. None for winners.
                        "stop_efficiency":(round(abs(pts)/abs(pos.entry_premium-pos.sl_price),2)
                                           if pnl<0 and pos.entry_premium!=pos.sl_price else ""),
                        # mfe/mae ratio: how favourable vs adverse the trade went
                        "mfe_to_mae_ratio":(round(pos.mfe_pts/abs(pos.mae_pts),2)
                                            if pos.mae_pts and pos.mae_pts!=0 else ""),
                    })
                    if tg_on():
                        e = "✅" if pnl>=0 else "❌"
            ...
            # atr_slope: linear trend of recent ATR (rising = move building,
            # falling = chop spike fading). Chop-discriminator, measured only.
            atr_hist.append(ind.get("atr", 0))
            if len(atr_hist) > 10:
                atr_hist.pop(0)
            atr_slope = ""
            if len(atr_hist) >= 5:
                atr_slope = round(float(np.polyfit(range(len(atr_hist)), atr_hist, 1)[0]), 3)
            log_scan({
                "datetime":now.strftime("%Y-%m-%d %H:%M:%S"), "nifty_ltp":round(ltp,1),
                ...
                # chop-analysis (measure before wiring): rvol already computed in ind
                "rvol":ind.get("rvol",""), "atr_slope":atr_slope,
            })

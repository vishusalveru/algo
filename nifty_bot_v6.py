"""
=============================================================
  Nifty 50 Scalping Bot v6 — Enhanced Paper Trading Build
  ─────────────────────────────────────────────────────────
  PURPOSE : Paper trading on live Upstox API to test all
            strategies with maximum data capture to refine
            for any type of market day.

  ALL v5 FIXES RETAINED (F1–F31)

  NEW FIXES IN v6 (from May 11 2026 CSV post-mortem):

  [V6-F1]  VWAP resets intraday at 09:15 each day.
           v5 bug: prev_vwap (23998) used as anchor all day
           instead of fresh intraday VWAP. Fixed: intraday
           VWAP now built from first candle of the day.

  [V6-F2]  ORB max range cap: skip ORB if range > 100pts.
           v5 bug: 123pt ORB range on May 11 — too wide for
           meaningful breakout. Signals fired inside noise.

  [V6-F3]  StratTracker PERFECT_CONF override: if conf=10/10
           allow trade even after consecutive losses.
           Also: HIGH_CONF_REENTRY lowered from 8→9 for
           post-loss re-entries (was too loose at 8).

  [V6-F4]  Counter-trend RSI relaxed: allow counter-trend if
           RSI < 65 (not just extreme <30/>70). Missed the
           entire 11:03 bullish recovery on May 11 because
           RSI was 62 — not "extreme" enough under v5 rules.

  [V6-F5]  RSI overbought SHORT block raised: 35→40.
           Trade 1 (May 11) entered bearish PE at RSI 76 —
           heavily overbought for a short. Block raised.

  [V6-F6]  FVG strong threshold raised: min size 5pts.
           v5 was taking 2pt FVGs (noise on Nifty).

  [V6-F7]  Market condition re-check before every strategy:
           Always evaluate live RSI, VWAP, EMA, trend fresh
           at signal time — not cached from scan start.

  [V6-F8]  New entry signal: BOS (Break of Structure).
           Detects swing high/low breaks on 5m timeframe.
           Strong directional signal missed by v5.

  [V6-F9]  New entry signal: ORPH/ORPL (Opening Range
           Previous High/Low). Uses PDH/PDL as breakout
           levels in addition to ORB. Better for trending
           gap-up / gap-down days.

  [V6-F10] New entry signal: RSI Divergence.
           Detects price making new high/low while RSI
           diverges — early reversal warning captured.

  [V6-F11] Dynamic SL/Target based on ATR.
           v5 fixed SL=10 / Target=15 regardless of
           volatility. v6: SL = 0.8x ATR, Target = 1.2x ATR
           (min 1:1.5 RR always maintained).

  [V6-F12] Paper trade max trades uncapped (was 999 already).
           Added per-strategy trade count in scan CSV for
           richer strategy-level analysis.

  [V6-F13] Day type classifier: TRENDING / RANGEBOUND /
           VOLATILE / GAP day. Logged in scan CSV.
           Strategies enabled/disabled per day type to
           survive any market condition.

  [V6-F14] Enhanced skip log: now logs WHY each strategy
           signal was seen but skipped — helps refine
           thresholds from data.

  [V6-F15] PERFECT_CONF (10/10) Telegram alert: sends a
           special 🔥 HIGH PRIORITY alert when any signal
           scores 10/10 — even if trade is skipped.

  STRATEGIES (12 — 3 NEW):
  1.  StrongFVG    — FVG + BOS retest at gap edge (async)
  2.  ORB+EMA      — Opening range breakout + EMA alignment
  ...
  8.  SuperTrend   — ATR-based fresh trend flip
  9.  CPR          — Central Pivot Range breakout/breakdown
  10. BOS          — [NEW] Break of Structure swing H/L
  11. ORPH_ORPL    — [NEW] PDH/PDL breakout on gap days
  12. RSIDivergence— [NEW] Price/RSI divergence reversal
  ...
  • MTF 3/3 agree     : +1
  • RSI factor        : ±1
  PERFECT (10/10)     : 🔥 special alert

  DAY TYPES (auto-classified each morning):
  • TRENDING    — directional strategies prioritised
  • RANGEBOUND  — mean-reversion strategies prioritised
  • VOLATILE    — reduced capital, wider SL
  • GAP_UP/DOWN — ORPH_ORPL + ORB+EMA prioritised

  DATA   : Upstox Live API (paper trading, no orders placed)
  LOGS   : scan_log_v6.csv, trade_log_v6.csv, skip_log_v6.csv
  STATE  : bot_state.json (daily P&L + manual bias persistence)
  ALERTS : Telegram with /status /report /bias /news /help
=============================================================
"""
...
# CSV log file names (v6)
SCAN_LOG_FILE  = f"scan_log_v6_{datetime.date.today()}.csv"
TRADE_LOG_FILE = f"trade_log_v6_{datetime.date.today()}.csv"
SKIP_LOG_FILE  = f"skip_log_v6_{datetime.date.today()}.csv"
...
def run():
    ...
    while True:
        ...
        if do_scan:
            ...
            tg(icon, f"NIFTY v6 SCAN {now.strftime('%H:%M')}", [
...
if __name__ == "__main__":
    try:
        ...
        log.info("Nifty Bot v6 stopped by user")
        send_telegram("⏹ Nifty Bot v6 stopped (KeyboardInterrupt)")
    except Exception as e:
        log.error(f"Fatal error: {e}", exc_info=True)
        send_telegram(f"🚨 Nifty Bot v6 CRASHED: {e}")
